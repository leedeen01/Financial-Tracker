...
const FriendList = ({ loggedInUser, fetchLoggedInUser }: FriendListProps) => {
  ...
  const calculateBalance = (friend: User): number => {
    const sentAmount = friend.topay
      .filter((item: FriendsExpenseRequestBody) => item.sendMoney === loggedInUser._id)
    ...
    const receivedAmount = friend.topay
      .filter((item: FriendsExpenseRequestBody) => item.receiveMoney === loggedInUser._id)
      .filter((item: FriendsExpenseRequestBody) => item.status === "accepted")
      .map((item: FriendsExpenseRequestBody) => parseInt(item.amount))
      .reduce((accumulator, currentValue) => accumulator + currentValue, 0);
              ...
              <div className="card h-md-100">
                <div className="card-body d-flex flex-row justify-content-between align-items-center">
                  ...
                  <div className="d-flex flex-column justify-content-between align-items-center w-25">
                    ...
                    <p className={`${calculateBalance(friend) >= 0 ? 'they-owe' : 'you-owe'}`}>${Math.abs(calculateBalance(friend)).toFixed(2)}</p>
                  </div>
          ...
          {friendRequestDetails.map((request) => (
            ...
            <div className="card h-md-100">
              <div className="card-body d-flex flex-row justify-content-between align-items-center">
                ...
                <button onClick={() => handleAcceptRequest(request._id!)} className="friend-button friend-button-accept">
                ...
                <button onClick={() => handleDeleteRequest(request._id!)} className="friend-button friend-button-reject">
                  <svg viewBox="0 0 16 16" height="16" width="16" xmlns="http://www.w3.org/2000/svg" className="icon">
